...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert self.can_contain(place.ant) ^ place.ant.can_contain(self) , 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.place:
            cnt = 0
            cur = self.place
            while cur and not cur.is_hive:
                if self.lower_bound <= cnt <= self.upper_bound and cur.bees:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        laser = self.damage - 0.25 * distance - self.insects_shot * 0.0625
        return laser if laser > 0 else 0
        # END Problem Optional 2
