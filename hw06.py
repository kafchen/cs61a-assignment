...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        time_value = Mint.present_year - self.year - 50
        return self.cents + (time_value if time_value > 0 else 0)
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            nv = VirFib(self.value + self.p + self.pp)
        nv.pp = nv.p
        nv.p = self.value
        return nv
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            return t.label
        sub_min = min([bst_min(b) for b in t.branches])
        return min(t.label,sub_min)

    def bst_max(t):
        if t.is_leaf():
            return t.label
        sub_max = min([bst_max(b) for b in t.branches])
        return max(t.label, sub_max)
