...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if temp == 1:
            ...
            outcomes += temp
        num_rolls -= 1
    return 1 if sow_sad else outcomes
...
def play(strategy0, strategy1, update,
    ...
    "*** YOUR CODE HERE ***"
    while True:
        if game_over(score0,score1,goal):
            break
        if who == 0:
...
def game_over(score0, score1, goal):
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    best_rolls,max = 1,0.0
    make_i_rolls = make_averaged(roll_dice, total_samples)
    for i in range(1,11):
        outcomes = make_i_rolls(i,dice)
        if outcomes > max:
            max = outcomes
            best_rolls = i
    return best_rolls
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    pig_tails = tail_points(opponent_score)
    if pig_tails >= threshold:
        return 0
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    gains = square_update(0,score,opponent_score)
    if gains - score >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11


def final_strategy(score, opponent_score,threshold=12,goal=100):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if goal-score <= 3:
        return 10
    elif square_strategy(score,opponent_score,threshold) == 0:
        return 0
    elif score - opponent_score > goal - score:
        return 3
    else:
        return 6
